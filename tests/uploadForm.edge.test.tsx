...
describe('UploadForm edge cases', () => {
  ...
  it('handles drag-and-drop file selection', async () => {
    ...
    dropzone.dispatchEvent(new DragEvent('drop', { bubbles: true, cancelable: true, dataTransfer } as any));

    await waitFor(() => expect(screen.getByText(/dragged.pdf/i)).toBeInTheDocument());
  });
  ...
  it('shows error when parsed PDF has no content (empty file)', async () => {
    ...
    const file = new File([''], 'empty.pdf', { type: 'application/pdf', size: 0 });
    await userEvent.upload(pdfInput, file);

    const submit = screen.getByRole('button', { name: /Begin Synthesis/i });
    await userEvent.click(submit);

    await waitFor(() => expect(require('sonner').toast.error).toHaveBeenCalledWith(expect.stringContaining('Failed to parse PDF')));
  });
  ...
  it('handles network failure mid-upload (uploadBookAsset throws)', async () => {
    ...
    (parsePDFFile as jest.Mock).mockResolvedValue({ content: ['p1'], cover: 'https://example.com/cover.png' });
    (uploadBookAsset as jest.Mock).mockRejectedValueOnce(new Error('Network error during upload'));
    ...
    const file = new File(['d'], 'sample.pdf', { type: 'application/pdf', size: 1024 });
    await userEvent.upload(pdfInput, file);
