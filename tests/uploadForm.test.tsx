...
const { checkBookExists, uploadBookAsset, createBookWithSegments } = require('@/lib/actions/book.actions');
...
describe('UploadForm', () => {
  ...
  it('renders empty state and accepts a PDF file', async () => {
    ...
    const file = new File(['dummy content'], 'sample.pdf', { type: 'application/pdf', size: 1024 });
    const input = screen.getByLabelText(/Upload PDF \*/i, { selector: 'input' }) || document.querySelector('input[type=file]');
  ...
  it('validates invalid file types and large files', async () => {
    ...
    const exeFile = new File(['exe'], 'malicious.exe', { type: 'application/x-msdownload', size: 100 });
    ...
    // Large file
    const bigFile = new File([new ArrayBuffer(60 * 1024 * 1024)], 'big.pdf', { type: 'application/pdf', size: 60 * 1024 * 1024 });
    await userEvent.upload(pdfInput, bigFile);
    await userEvent.click(submit);
    await waitFor(() => {
      expect(screen.getByText(/PDF file must be less than/i)).toBeInTheDocument();
    });
  });

  it('handles a successful upload flow', async () => {
    checkBookExists.mockResolvedValue({ exists: false });
    parsePDFFile.mockResolvedValue({ content: ['page1'], cover: 'https://example.com/cover.png' });
    uploadBookAsset.mockResolvedValue({ success: true, data: { url: 'https://cdn/sample.pdf', pathname: '/sample.pdf' } });
    // Second call for cover
    uploadBookAsset.mockResolvedValueOnce({ success: true, data: { url: 'https://cdn/sample.pdf', pathname: '/sample.pdf' } }).mockResolvedValueOnce({ success: true, data: { url: 'https://cdn/cover.png', pathname: '/cover.png' } });
    createBookWithSegments.mockResolvedValue({ success: true, data: { book: { slug: 'test-book' } } });

    // mock fetch used for parsedPDF.cover
    global.fetch = jest.fn().mockResolvedValue({ blob: async () => new Blob(['img']) });
    ...
    const file = new File(['dummy'], 'sample.pdf', { type: 'application/pdf', size: 1024 });
    await userEvent.upload(pdfInput, file);
    ...
    const radio = screen.getByLabelText(/Dave/i) || document.querySelector('input[value="dave"]');
