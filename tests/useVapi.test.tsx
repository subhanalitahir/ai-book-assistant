...
const SampleBook = { _id: 'book_1', title: 'Test Book', author: 'Author', persona: 'dave' } as any;
...
describe('useVapi hook', () => {
  ...
  it('starts a session successfully (happy path)', async () => {
    (startVoiceSession as jest.Mock).mockResolvedValue({ success: true, sessionId: 'sess_1' });
  ...
  it('handles startVoiceSession returning failure', async () => {
    (startVoiceSession as jest.Mock).mockResolvedValue({ success: false, error: 'no-capacity' });
    render(<TestComponent book={SampleBook} />);
  ...
  it('handles Vapi.start throwing (microphone permission denied)', async () => {
    (startVoiceSession as jest.Mock).mockResolvedValue({ success: true, sessionId: 'sess_2' });
    mockStart.mockImplementationOnce(() => { throw new Error('Microphone permission denied'); });
    ...
    await waitFor(() => {
      expect(mockStart).toHaveBeenCalled();
      expect(screen.getByText(/error:/i)).toHaveTextContent('Failed to start session. Please try again.');
    });
  });

  it('stop() calls vapi.stop()', async () => {
    (startVoiceSession as jest.Mock).mockResolvedValue({ success: true, sessionId: 'sess_3' });
    render(<TestComponent book={SampleBook} />);
...
jest.mock('@vapi-ai/web', () => {
  return jest.fn().mockImplementation(() => ({ start: startMock, stop: stopMock }));
});

jest.mock('@/lib/actions/session.actions', () => ({
  startVoiceSession: jest.fn().mockResolvedValue({ success: true, sessionId: 'sess_1' }),
}));
...
describe('useVapi hook', () => {
  ...
  it('starts a call and calls Vapi.start with book context', async () => {
    const book = { _id: 'book_1', title: 'Title', author: 'Author', persona: 'dave' } as any;
  ...
  it('handles SDK errors (microphone permission denied)', async () => {
    ...
    const book = { _id: 'book_2', title: 'T', author: 'A', persona: 'dave' } as any;
    ...
    expect(result.current.limitError).toMatch(/Failed to start session|Microphone permission denied/i);
  });

  it('does not start if user not logged in', async () => {
    jest.mocked(require('@clerk/nextjs').useAuth).mockImplementation(() => ({ userId: null }));
    const book = { _id: 'book_3', title: 'T', author: 'A', persona: 'dave' } as any;
    const { result } = renderHook(() => useVapi(book));
